...
def test_router_instantiates_with_config():
    ...
    # Router will try to initialize providers, which may fail
    # if credentials not set. That's OK for this test.
    try:
...
def test_router_fallback_disabled():
    ...
    try:
        ...
        # Fallback provider should not be initialized
        assert router.fallback is None
    except RuntimeError:
        pass


def test_router_get_provider_status():
    """Verify router can report provider status."""
    config = RuntimeConfig(
        primary_provider="anthropic",
        secondary_provider="openrouter",
        routing_mode="failover",
    ...
    try:
        ...
        assert status["routing_mode"] == "failover"
    except RuntimeError:
        pass
